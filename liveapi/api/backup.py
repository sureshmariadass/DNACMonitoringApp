...
def get_token():
    post_url = "https://10.91.91.172/dna/system/api/v1/auth/token"
...
def get_device_list(token):
    url="https://10.91.91.172/dna/intent/api/v1/network-device"
...
def get_device_detail(token,macAd):
    url="https://10.91.91.172/dna/intent/api/v1/device-detail/?searchBy="+macAd+"&identifier=macAddress"
    header = {'x-auth-token': token, 'content-type' : 'application/json'}
    resp = requests.get(url, headers=header,verify=False, timeout=DEF_TIMEOUT)
...
if __name__ == "__main__":
    ...
    for mac in device_list['response']:
        ...
        data.append(dev_det)
        if 'cpu' in dev_det['response']:
            print(dev_det['response']['cpu'])
    for x in data:
        if 'cpu' in x['response']:
            print(x['response'])